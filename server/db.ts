...
export async function getAllGalleryImages() {
  ...
  const result = await db
    .select()
    .from(galleryImages)
    .where(eq(galleryImages.isPublished, 1))
...
export async function deleteGalleryImage(id: number) {
  ...
  if (image && image.imageKey) {
    try {
      // Import imageBlobs dynamically to avoid circular dependency issues if any
      const { imageBlobs } = await import("../drizzle/schema");
      await db.delete(imageBlobs).where(eq(imageBlobs.key, image.imageKey));
    } catch (error) {
      console.warn("[Database] Failed to delete image blob:", error);
    }
  }
